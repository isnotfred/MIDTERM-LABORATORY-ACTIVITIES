...
int main() {
    ...
    double originalPrice, markupRate, salesTax, storeRate, electricityBill, waterBill, salary, priceExTax, priceAfterTax, expenses, revenue;
    ...
    expenses = (originalPrice * itemQuantity) + storeRate + electricityBill + waterBill + (numEmployee * salary);
    ...
    if (originalPrice && markupRate && salesTax && storeRate && electricityBill && waterBill && salary && itemQuantity && numEmployee >= 0) {
        ...
        cout << "Store Monthly Rate: " << storeRate << endl;
